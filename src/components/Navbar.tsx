...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#0b1329]/95 backdrop-blur-md border-b border-blue-100 dark:border-slate-800 shadow-sm transition-colors duration-200">
      {/* Top Banner / Department bar & Device Mode Controls */}
      <div className="bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-700 dark:from-[#0d1b3e] dark:via-[#11224d] dark:to-[#172554] text-white text-xs py-1 px-3 sm:px-6">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 font-medium">
            <span className="inline-block w-2 h-2 rounded-full bg-emerald-400 animate-pulse shrink-0" />
            <span className="text-[11px] sm:text-xs truncate">{t.departmentSub}</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2.5 space-y-2.5">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          >
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-tr from-blue-700 to-blue-500 text-white flex items-center justify-center shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform duration-200">
              <Cpu className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg sm:text-xl font-black tracking-tight text-blue-900 dark:text-white">
                  {t.platformName}
                </h1>
          ...
          <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
            ...
            >
              {theme === 'dark' ? (
                <>
                  ...
                  <span className="hidden sm:inline font-bold">{t.lightMode}</span>
                </>
              ) : (
                <>
                  <Moon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-600 shrink-0" />
                  <span className="hidden sm:inline font-bold">{t.darkMode}</span>
                </>
              )}
            </button>
            ...
            <button
              id="btn-admin-panel"
              onClick={onOpenAdmin}
              className={`px-2.5 sm:px-3.5 py-1.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer ${
                isAdminUnlocked
            ...
            >
              {isAdminUnlocked ? (
                <>
                  ...
                  <span className="hidden sm:inline">{t.adminModeActive}</span>
                  <span className="sm:hidden">{language === 'ar' ? 'المسؤول' : 'Admin'}</span>
                ...
                <>
                  ...
                  <span className="sm:hidden">{language === 'ar' ? 'إضافة ملزمة' : 'Add Note'}</span>
                </>
              )}
            </button>
            ...
            <button
              ...
              className="px-2.5 sm:px-3.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-500 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 text-xs font-bold flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer"
              title="تغيير لغة المنصة"
            >
              <Languages className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-600 dark:text-blue-400 shrink-0" />
              <span className="font-bold">{language === 'ar' ? 'EN' : 'العربية'}</span>
            </button>
          </div>
        </div>

        {/* Row 2: Dedicated Navigation Tabs Bar (Spacious, prominent, zero clutter) */}
        <div className="bg-slate-100/90 dark:bg-slate-900/90 p-1.5 rounded-2xl border border-slate-200/80 dark:border-slate-800 flex items-center gap-1 sm:gap-1.5 overflow-x-auto scrollbar-none shadow-xs">
          <button
            id="nav-tab-lectures"
            onClick={() => onTabChange('lectures')}
            className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-bold flex items-center gap-1.5 whitespace-nowrap transition-all shrink-0 cursor-pointer ${
              activeTab === 'lectures'
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs sm:text-sm font-black flex items-center gap-2 whitespace-nowrap transition-all shrink-0 cursor-pointer relative ${
              activeTab === 'quizzes'
